...
// Keep in sync with src/lib/gallery-slide-copy.ts
...
function getGallerySlideSummary(item) {
  ...
  if (t.includes("vulture")) {
    ...
    if (t.includes("perched")) return "Silhouetted against the wide savanna sky.";
    return pick(seed, [
  ...
  if (t.includes("elephant")) {
    ...
    if (t.includes("calves") || t.includes("family")) return "Calves tucked close in the family march.";
  ...
  if (t.includes("warthog")) {
    return pick(seed, ["A warthog family trotting through the grass.", "Tusks up, tail high — off to the burrow."]);
  }
  ...
  if (t.includes("dance") || t.includes("drum") || t.includes("weaving") || t.includes("stone town")) {
    ...
    return pick(seed, ["Rhythm, colour, and community in motion.", "Drums and dance that carry generations."]);
  }
...
while ((match = entryRe.exec(raw)) !== null) {
  ...
  item.summary = getGallerySlideSummary(item);
  items.push(item);
}
...
console.log(`Refreshed summaries for ${items.length} gallery items.`);
